...
| Phase 5 | 完整资料处理能力 | 3～5 周 | 更多格式、URL、关联图片展示、一次性文件夹导入 |
...
当前状态（2026-09-07）：第一批 HTML/HTM、第二批安全 OOXML/PPTX/XLSX、第三批受限 URL 快照与第四批有界文件夹递归导入均已通过桌面验收。文件夹增量监听已由项目负责人调整为[第二期功能](folder-monitoring-backlog.md)，不属于一期 Phase 5。交付边界见 [Phase 5 交付记录](phase-5-delivery.md)。
...
- 表格、代码、问答和长文档专用分块。
- [搜索关联图片展示](image-preview-backlog.md)：不做 OCR，搜索命中文字后展示同页或相邻结构中的原图。
- 批量重试、暂停和取消任务。
...
6. 作为非阻塞质量工作，将评测扩展到 200～500 条，并在 1 万真实 E5 分块下记录性能指标；代码签名与 macOS/Linux 生命周期验证继续按发布计划开展。
