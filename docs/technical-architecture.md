...
- Core 集中维护应用数据库与索引构建契约版本，并在版本握手中与 Core/API 版本一并返回。应用数据库只做确定性的前向迁移并拒绝更高版本；派生索引不原地迁移，而是在活动文件旁重建后原子切换。桌面端只接受与其构建时完全匹配的运行时组合，详见 [ADR 0020](decisions/0020-runtime-version-compatibility.md)。
