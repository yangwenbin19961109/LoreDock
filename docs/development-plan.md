...
当前状态：前十二批 UI 与桌面能力已接入真实 Core，并按既有渲染图形成左侧导航、中间资料列表、右侧预览详情的三栏结构；资料管理、检索、保留引用偏移的安全结构化预览、Core keyset 游标分页、首次启动引导、明暗主题、常规设置，以及带真实进度和断点续传的本地模型后台任务已经落地。第一次人工验收已发现拖放、结果定位、标题栏布局、检索可解释性、搜索重置和窗口适配共 6 项问题，Phase 3 当前处于验收整改而非最终完成状态，按 [Phase 3 阶段验收问题与修复排期](phase-3-acceptance-issues.md) 分三批处理。Tauri 已支持动态端口、进程令牌、版本握手、优雅关闭、三次有界异常恢复、脱敏诊断和人工重启；Windows 独立 Core、resource 装配、MSI 与 NSIS 构建及发行目录故障烟雾测试已通过，干净虚拟机验收矩阵已有受保护的自动脚本。多模型切换、PDF/DOCX 原始版面还原、代码签名、虚拟机实测以及 macOS/Linux 生命周期验证仍待后续批次完成。
