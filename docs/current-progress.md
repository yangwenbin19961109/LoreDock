...
- 记录日期：2026-09-04
- 当前分支：`main`
- 当前里程碑：Phase 4 本轮人工验收通过；新版 Core 已启动并迁移至 v5，最近使用/收藏待桌面人工验收；Cursor 实机验收继续暂缓
...
- 最近验证：Core 93 项通过、1 项系统测试默认跳过；Web 37 项单元测试、7 项浏览器测试通过；Pyright、ESLint、Web 类型/构建与相关格式检查通过。保留 Starlette/httpx 弃用警告及桌面链接器信息警告。本次提交整理不把历史检查描述为全部重跑。
- 下一步：人工验收最近使用/收藏的打开、切换、重启保留与删除清理；通过后再确认下一开发阶段。Cursor、干净安装矩阵、跨平台验证及排序调优保留原排期。
- 测试程序保持运行供验收。本次仅提交，不推送；不包含安装包、模型、数据库、备份、私人资料、凭据或运行日志。
