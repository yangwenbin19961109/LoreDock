...
Phase 0～2 已建立工程、资料管理和基础检索闭环，Phase 2.5 的功能主路径与 100 条质量基线已经完成，Phase 3 已完成前十二批桌面功能、安全结构化预览、明暗主题走查、Windows 本地验收包和虚拟机验收脚本。下一轮按以下顺序继续：
...
5. 在干净 Windows 虚拟机完成安装、升级、卸载和数据保留矩阵，再开展代码签名与 macOS/Linux 生命周期验证。
6. 冻结只读 MCP 结果契约并进入 Phase 4，实现与 HTTP 共用应用服务的 MCP 工具和 Agent 接入向导。
