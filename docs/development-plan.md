...
| Phase 5 | 完整资料处理能力 | 3～5 周 | 更多格式、URL、关联图片展示、文件夹监听 |
...
当前状态（2026-09-07）：第一批 HTML/HTM 本地文件导入、预览定位和导入进度反馈已通过桌面验收。第二批采用 [ADR 0015](decisions/0015-ooxml-readonly-ingestion.md) 的受限只读 OOXML 方案；安全容器基础层与 PPTX 已完成且通过桌面验收，XLSX 工作表、单元格值/坐标与页码解析已实现，正在进行自动回归和桌面验收。URL 和文件夹监听留待后续独立批次。交付边界见 [Phase 5 交付记录](phase-5-delivery.md)。
