...
1. 对 Phase 3 整改批次 A、B 做最后一轮桌面人工复验：结果驱动预览定位、搜索重置、长文件名操作区和两种窗口状态下的响应式布局。
2. 在干净 Windows 虚拟机完成安装、升级、卸载和数据保留矩阵。
3. 人工复验与安装矩阵通过后，标记 Phase 3 Windows MVP 完成并冻结其交付记录。
4. 进入 Phase 4，冻结并实现只读 MCP 结果契约、服务与 Codex/Cursor 接入向导。
